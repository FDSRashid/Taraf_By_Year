...
def plot_taraf_map(min_year = 0, max_year = 400):
  ...
  fig = px.scatter_mapbox(data_frame = filtered, lat = filtered.geometry.y, lon = filtered.geometry.x,size = filtered.Taraf,color = filtered.Taraf, title = 'Number of Tarafs in Place', opacity = .5, zoom = 0)
